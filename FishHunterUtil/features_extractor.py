...
def get_dataset_features(dataset_path):
    # clean dataset
    # cleaning HTML
    html_text = open(dataset_path+"/index.html", "r", encoding="utf-8", errors='ignore').read()
    ...
    # Save to clean.html
    with open(dataset_path+"/clean.html", "w", encoding="utf-8", errors='ignore') as f:
        f.write(lxml.html.tostring(html, encoding="unicode"))

    css_propval = extract_css_propval_from_file(dataset_path+"/assets/__clean__.css", document_root=f"{dataset_path}/")
    # Save css_propval to css.json
    with open(dataset_path+"/css.json", "w", encoding="utf-8", errors='ignore') as f:
        json.dump(css_propval, f, indent=4, sort_keys=True)

    # GET TEXT FEATURES
    html_text = open(dataset_path+"/clean.html", "r", encoding="utf-8",errors='ignore').read()
